...
namespace csp
{
    ...
    inline void tsp::SetWeightMatrixLateralBM()
    {
        ...
        for (int v_idx = 0; v_idx < _numNeuronsPrincipal; v_idx++)
        {
            for (int h_idx = 0; h_idx < _numNeuronsPrincipal; h_idx++)
            {
                ...
                { // same WTA===============================================================
                    ...
                    { // in the same_WTA but btw different cities
                ...
                else if (fabs((v_WTA) - (h_WTA)) == 1 || fabs((v_WTA) - (h_WTA)) == (_numCity - 1))
                { // adjacent WTA
                    if (v_city == h_city)
                    { // adjacent WTA - same city = inhibition!
                    ...
                    { // THE MOST IMPORTANT PART OF THE TRAVELING SALESMAN PROBLEM
                        weight_matrix[v_idx][h_idx].Gp = _b + (1 - distance / max_distance) * _grad;
                        weight_flag = "*";
                    }
                } // different WTA not neighbor===============================================================
                else if (fabs((v_WTA % _numCity) - (h_WTA % _numCity)) != 1)
                { // non-adjacent WTA networks
                    if (v_city == h_city)
                    {
                        if (_InhibitNonAdjWTASameCities > 0)
                        { // inhibition btw same cities
        ...
        {
            for (int h_idx = 0; h_idx < _numCity; h_idx++)
            {
                weight_matrix[v_idx][h_idx * _numCity + v_idx % _numCity].Gm = 0.01;
            }
